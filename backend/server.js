...
//   return Object.keys(balances).map((customer) => ({
//     customer,
//     cards: Object.keys(balances[customer]).map((cardNumber) => ({
//       cardNumber,
//       balance: parseFloat(balances[customer][cardNumber].toFixed(3)), 
//     })),
//   }));
// }
...
//       transactions.push({
//         customer: row['Account Name'] || 'Unknown Account Name',  
//         cardNumber: row['Card Number'] || 'Unknown Card Number',  
//         amount: parseFloat(row['Transaction Amount']) || 0,  
//         type: row['Transaction Type'] || 'Unknown',  
//         description: row['Description'] || 'No Description',  
//         targetCardNumber: row['Target Card Number'] || 'N/A',  
//       });
//     })
//     .on('end', () => {
//       allTransactions = [...allTransactions, ...transactions];

//       const balanceReport = calculateBalances(allTransactions);

//       res.json({ balanceReport, allTransactions, uploadedFiles });
//     })
//     .on('error', (err) => {
//       console.error('Error processing CSV:', err);
//       res.status(500).json({ error: 'Failed to process file' });
